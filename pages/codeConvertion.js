...
const codeConvertion = () => {
  ...
  useEffect(() => {
    const fetchDigital = async () => {
      await setResult(
        getCookie("code") == undefined ? null : JSON.parse(getCookie("code"))
      );
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const arr =
      getCookie("code") == undefined ? [] : JSON.parse(getCookie("code"));
    console.log("entered");
    const res = await Axios.post("/api/codeConversion", { prompt: input });
    const obj = { question: input, answer: JSON.stringify(res.data[0].text) };
    await arr.push(obj);
    await setCookie("code", JSON.stringify(arr), {
      maxAge: 1000000,
    });
    console.log(res.data[0].text);
    console.log("done");
    console.log(JSON.parse(getCookie("code")));
    setLoading(false);
    setResult(JSON.parse(getCookie("code")));
  };
