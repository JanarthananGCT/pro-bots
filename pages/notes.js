...
const Notes = () => {
  ...
  useEffect(() => {
    const fetchDigital = async () => {
      await setResult(
        getCookie("notes") == undefined
          ? null
          : JSON.parse(getCookie("notes"))
      );
    };
  ...
  const handleSubmit = async (e) => {
    ...
    const arr =
      getCookie("notes") == undefined
        ...
        : JSON.parse(getCookie("notes"));
    console.log("entered");
    const res = await Axios.post("/api/notes", { prompt: input });
    console.log(res);
    const obj = { question: input, answer: JSON.stringify(res.data[0].text) };
    await arr.push(obj);
    await setCookie("notes", JSON.stringify(arr), {
      maxAge: 1000000,
    });
    ...
    setLoading(false);
    setResult(JSON.parse(getCookie("notes")));
  };
