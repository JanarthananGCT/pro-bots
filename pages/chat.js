...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchDigital = async () => {
      await setResult(
        getCookie("chat") == undefined ? null : JSON.parse(getCookie("chat"))
      );
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    const arr =
      getCookie("chat") == undefined ? [] : JSON.parse(getCookie("chat"));
    console.log("entered");
    const res = await Axios.post("/api/chatGPT", { prompt: input });
    const obj = { question: input, answer: JSON.stringify(res.data[0].text) };
    await arr.push(obj);
    await setCookie("chat", JSON.stringify(arr), {
      maxAge: 1000000,
    });
    console.log(res.data[0].text);
    console.log("done");
    console.log(JSON.parse(getCookie("chat")));
    setLoading(false)
    setResult(JSON.parse(getCookie("chat")));
  };
