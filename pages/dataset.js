...
import DataTemplate from "../components/dataTemp";
...
const Dataset = () => {
  ...
  useEffect(() => {
    const fetchDigital = async () => {
      await setResult(
        getCookie("dataset") == undefined
          ? null
          : JSON.parse(getCookie("dataset"))
      );
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const arr =
      getCookie("dataset") == undefined ? [] : JSON.parse(getCookie("dataset"));
    console.log("entered");
    const res = await Axios.post("/api/dataset", { prompt: input });
    console.log(res);
    const obj = { question: input, answer: JSON.stringify(res.data[0].text) };
    await arr.push(obj);
    await setCookie("dataset", JSON.stringify(arr), {
      maxAge: 1000000,
    });
    console.log(res.data[0].text);
    console.log("done");
    console.log(JSON.parse(getCookie("dataset")));
    setLoading(false);
    setResult(JSON.parse(getCookie("dataset")));
  };
          ...
          <div className="mt-8 mx-4 pb-20 flex flex-col justify-center items-center">
            {loading ? (
              ...
              <div>
                ...
                {result ? (
                  result.map((item) => (
                    <DataTemplate que={item.question} ans={item.answer} />
                  ))
                ) : (
                  <div className="flex justify-center items-center">
                    <Player
