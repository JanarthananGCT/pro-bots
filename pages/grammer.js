...
const Grammer = () => {
  ...
  useEffect(() => {
    const fetchDigital = async () => {
      await setResult(
        getCookie("grammer") == undefined ? null : JSON.parse(getCookie("grammer"))
      );
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const arr =
      getCookie("grammer") == undefined ? [] : JSON.parse(getCookie("grammer"));
    console.log("entered");
    const res = await Axios.post("/api/grammer", { prompt: input });
    console.log(res);
    const obj = { question: input, answer: JSON.stringify(res.data[0].text) };
    await arr.push(obj);
    await setCookie("grammer", JSON.stringify(arr), {
      maxAge: 1000000,
    });
    console.log(res.data[0].text);
    console.log("done");
    console.log(JSON.parse(getCookie("grammer")));
    setLoading(false);
    setResult(JSON.parse(getCookie("grammer")));
  };
