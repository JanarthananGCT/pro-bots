...
import ImageTemplate from "../components/imageTemp";
...
const GenerateImage = () => {
  ...
  useEffect(() => {
    const fetchDigital = async () => {
      await setResult(
        getCookie("image") == undefined ? null : JSON.parse(getCookie("image"))
      );
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const arr =
      getCookie("image") == undefined ? [] : JSON.parse(getCookie("image"));
    console.log("entered");
    const res = await Axios.post("/api/dallE", { prompt: input });
    console.log(res);
    const obj = { question: input, answer: JSON.stringify(res.data) };
    await arr.push(obj);
    await setCookie("image", JSON.stringify(arr), {
      maxAge: 1000000,
    });
    console.log(res.data[0].url);
    console.log("done");
    console.log(JSON.parse(getCookie("image")));
    setLoading(false);
    setResult(JSON.parse(getCookie("image")));
  };
          ...
          <div className="mt-8 mx-4 pb-20 flex flex-col justify-center items-center">
            {loading ? (
              ...
              <div>
                ...
                {result ? (
                  result.map((item) => (
                    <ImageTemplate que={item.question} ans={item.answer} />
                  ))
                ) : (
                  <div className="flex justify-center items-center">
                    <Player
