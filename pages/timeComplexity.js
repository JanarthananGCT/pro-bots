...
const TimeComplexity = () => {
  ...
  useEffect(() => {
    const fetchDigital = async () => {
      await setResult(
        getCookie("timeComplexity") == undefined
          ? null
          : JSON.parse(getCookie("timeComplexity"))
      );
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const arr =
      getCookie("timeComplexity") == undefined ? [] : JSON.parse(getCookie("timeComplexity"));
    console.log("entered");
    const res = await Axios.post("/api/chatGPT", { prompt: input });
    console.log(res);
    const obj = { question: input, answer: JSON.stringify(res.data[0].text) };
    await arr.push(obj);
    await setCookie("timeComplexity", JSON.stringify(arr), {
      maxAge: 1000000,
    });
    console.log(res.data[0].text);
    console.log("done");
    console.log(JSON.parse(getCookie("timeComplexity")));
    setLoading(false);
    setResult(JSON.parse(getCookie("timeComplexity")));
  };
